...
public class PostBO {
	...
	public int updatePost(int postId, int userId, String userLoginId, 
		...
		if (post == null) { // 수정할 글이 존재하지 않는 경우 (없을 것 같지만 있을 수 있다)
		...
		if (file != null) {
			...
			// 업로드 성공하면 기존 이미지 제거
			if (imagePath != null && post.getImagePath() != null) { // 기존 이미지 있는 경우
				// 업로드가 실패할 수 있으므로 업로드가 된 후 제거
				fileManagerService.deleteFile(post.getImagePath());
			}
		}
		
		// DB update => imagePath가 없으면 mybatis에서 업데이트 하지 않도록 처리
	...
	public List<Post> getPostListByUserId(int userId, Integer prevId, Integer nextId) {
		...
		// 첫페이지일 때는 standardId가 null, 다음일때는 값이 있음
